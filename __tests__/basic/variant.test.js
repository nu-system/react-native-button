...
describe("枚举variant属性，判断结果是否正确",()=>{
    ...
    it(`ghost border和字体颜色以及loading颜色等于当前level色`,()=>{
        ...
        const tmp1= shallow(<NuButton loading variant="ghost" ></NuButton>).find("ActivityIndicator")
        expect(tmp1.prop("color")).toBe(userProps.levelColor)
    })
    it(`link 字体颜色以及loading颜色等于当前level色`,()=>{
        const tmp= shallow(<NuButton variant="link">ghost</NuButton>)
        const {theme , style, Pre, Append, ...userProps}=tmp.props()
        expect(userProps.textStyle.color).toBe(userProps.levelColor)

        const tmp1= shallow(<NuButton loading variant="link" ></NuButton>).find("ActivityIndicator")
        expect(tmp1.prop("color")).toBe(userProps.levelColor)
    })
})
